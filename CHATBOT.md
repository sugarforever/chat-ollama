...
# ChatOllama Web Chatbot and Knowledge-Base Platform

> ChatOllama is now focused on its Agent Runtime and CLI. This document preserves the setup and reference material for the existing Nuxt web chatbot, knowledge bases, voice chat, and administration features. Return to the [main Agent README](./README.md) for the current project direction.
...
characters. The application refuses to start when it is missing or uses a known
public placeholder. Changing it invalidates all existing login tokens.
