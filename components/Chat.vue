...
const onSend = async (data: ChatBoxFormData) => {
    ...
    // Determine which models to use - hijacked models take precedence
    const targetModels = data.hijackedModels && data.hijackedModels.length > 0
        ? data.hijackedModels
        : models.value

    const timestamp = Date.now()
    sendingCount.value = targetModels.length
    ...
    if (sessionInfo.value && targetModels.length > 0) {
        const { family, name: model } = parseModelValue(targetModels[0])
    ...
    try {
        ...
        // Send message to each target model - create separate loading message for each
...
function onOpenSettings() {
    modal.open(ChatSettings as any, {
        ...
        onUpdated: (data: any) => {
            ...
            knowledgeBaseInfo.value = data.knowledgeBaseInfo || null
            instructionInfo.value = data.instructionInfo || null
